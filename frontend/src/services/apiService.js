...
export const apiService = {
  ...
  getTrials: async (hospital) => {
    try {
      const params = hospital ? `?hospital=${encodeURIComponent(hospital)}` : '';
      const response = await axios.get(`${API_BASE_URL}/v2/trials${params}`);
      return response.data;
    } catch (error) {
  ...
  createTrial: async (trialData, hospital) => {
    try {
      const params = hospital ? `?hospital=${encodeURIComponent(hospital)}` : '';
      const response = await axios.post(`${API_BASE_URL}/v2/trials${params}`, trialData);
      return response.data;
    } catch (error) {
  ...
  getEligibleForDrug: async (drugName, hospital, { page = 1, pageSize = 50, tab = 'eligible', scope = 'global', search = '' } = {}) => {
    try {
      ...
      if (search) params.set('search', search);
      const response = await axios.get(`${API_BASE_URL}/v2/trials/${encodeURIComponent(drugName)}/eligible?${params}`);
      return response.data;
    } catch (error) {
  ...
  addPatientsToTrial: async (drugName, patientIds = []) => {
    try {
      const response = await axios.post(
        `${API_BASE_URL}/v2/trials/${encodeURIComponent(drugName)}/patients`,
        { patient_ids: patientIds }
      );
      return response.data;
    } catch (error) {
      console.error('Error adding patients to trial:', error);
      throw error;
    }
  },

  removePatientsFromTrial: async (drugName, patientIds = []) => {
    try {
      ...
      console.error('Error removing patients from trial:', error);
      throw error;
    }
  },

  // Get available patients for trial enrollment (not already enrolled, disease match)
  getAvailablePatientsForTrial: async (drugName, hospital = null) => {
    try {
      const params = hospital ? `?hospital=${encodeURIComponent(hospital)}` : '';
      const response = await axios.get(
        `${API_BASE_URL}/v2/trials/${encodeURIComponent(drugName)}/available-patients${params}`
      );
      return response.data;
    } catch (error) {
      console.error('Error getting available patients for trial:', error);
      throw error;
    }
  },

  autoEnrollPatients: async (drugName) => {
    try {
      const response = await axios.post(
        `${API_BASE_URL}/v2/trials/${encodeURIComponent(drugName)}/auto-enroll`
      );
      return response.data;
    } catch (error) {
      console.error('Error auto-enrolling patients:', error);
      throw error;
    }
  },
